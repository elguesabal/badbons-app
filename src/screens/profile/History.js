...
export default function History({ navigation }) {
	...
	return (
		<FlatList contentContainerStyle={history.scroll} data={scroll.events} keyExtractor={(_, i) => i.toString()} onEndReachedThreshold={0.2} ListFooterComponent={(scroll.loadingMore) ? <ActivityIndicator size="large" color="white" /> : null } onEndReached={() => requestGameHistory(navigation, setLoad, setIsLogin, openModal, scroll, setScroll)}
			renderItem={({ item, index }) => (
				<View key={index} style={history.containerEvent} >
					<Text style={history.titleEvent} >{item.event}</Text>
					{item.games.map((game, j) => (<Scoreboard key={j} style={history.scoreboard} game={game} />))}
